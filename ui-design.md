...
> **Status: design pass complete.** This file was the brief used for the UI design pass with Claude Design. The pass produced visual mockups now living in `ui-design/design_handoff/` — that folder is the **source of truth for visuals** going forward.
>
> Last updated: 2026-04-29 (after design-pass review and reconciliation).
>
> **Six tracked deviations from this brief** were accepted during the design review (see `ui-design/design_handoff/SPEC-DEVIATIONS.md`):
> - **D-001** `sold` is editable (not frozen) — sale can fall through; only `picked-up` and `not-sellable` are frozen.
> - **D-002** Inventory is the single action hub — bulk dialogs don't re-list selected lots.
> - **D-003** Bulk Change-state shows only shared legal transitions across the selection.
> - **D-004** Cataloging has an explicit **Print Label** button — Next saves but does not auto-print.
> - **D-005** Closed jobs grayed (visible-but-disabled) in pickers — implemented via `job.closed_at`.
> - **D-006** No separate Auctions page — Job CRUD lives on Customer detail; "view auction" = (Customer, Job) filter on Inventory.
>
> Additional accepted refinements: `sold → unassigned` transition added (sale fall-through); `in_progress` is a UI-only transient state during cataloging; AI `partial` status means at least one target field generated and at least one missing.
>
> The behavioral / schema impact of all of the above is reflected in `overview.md` and `docs/superpowers/specs/2026-04-29-v1-design.md`.
