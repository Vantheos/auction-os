...
> **Post-task extension (commit `98478e6`):** Two fixes from Task 9 review:
> - **PATCH delete-race 404 guard** in `api/lots/[id].ts`: if the lot is deleted by another request between the pre-check and the UPDATE, the UPDATE matches no rows and the handler now returns 404 instead of an empty body. Plus 2 new tests (`PATCH 404 on unknown id`, `DELETE 404 on unknown id`).
> - **Task 8 retrofit — POST returns joined DTO** in `api/lots/index.ts`: extracted `fetchLotJoined` helper and used it after the INSERT so the POST response carries the same `{customerName, jobNumber, customerId}` join shape that GET and PATCH return. Removes shape asymmetry between create-and-update for `useUpdateLot` (Task 18). Plus 1 new POST test asserting the joined fields.
...
> **Implemented as commit `c8bb24f` with two corrections to the snippet above:**
> - **State guard restricted to `assigned` only.** The plan code allowed `assigned` OR `sold`. Spec §4.2 explicitly says "Move to another auction *(only `assigned`)*" — sold lots belong to a buyer; falling-through goes via `sold → unassigned`, not a direct move. Implementation throws a typed `IllegalMoveError` (cleaner than the plan's string-prefix match) and translates to 422 `ILLEGAL_MOVE`.
> - **Third test rewritten.** The plan's `testDb.update(lot).set(...).where(({id}) => undefined as any)` was broken Drizzle syntax. The corrected test deterministically mutates the seed lot via `eq(lot.id, lotId)` and asserts 422 `ILLEGAL_MOVE` for a sold lot.
> - Added 4 more tests for office happy-path, 404 unknown id, missing destinationJobId, and sequential lot_number allocation. Total: 7.
> - Returns the joined-DTO shape via a local `fetchLotJoined` helper (third copy across `lots/index.ts`, `lots/[id].ts`, `lots/[id]/move.ts`). **Task 12 (bulk) should extract this to `api/_lib/lot-queries.ts`** when it becomes the fourth call site.
> - 23505 (lot_number race) handled as 409 `LOT_NUMBER_CONFLICT`. **23503 (FK violation when destination job doesn't exist) currently bubbles to 500 — also fix at Task 12.**
...
> **Implemented as commits `a435376` + `0892df2` with corrections:**
> - **`applyMove` state guard restricted to `assigned` only** (plan code allowed `sold`; spec §4.2 forbids).
> - **SAVEPOINT pattern via Drizzle nested `tx.transaction(async (sub) => {...})`** for `applyMove` — the plan's per-lot `try/catch` around `tx.update` was structurally broken (Postgres aborts the entire txn on any error, making per-lot recovery impossible without savepoints).
> - **Postgres error detection** uses `e.code ?? e.cause?.code` (Drizzle wraps). 23505 → `LOT_NUMBER_CONFLICT`; 23503 → `INVALID_DESTINATION` (per-lot results).
> - **Invalid `params.to`** (e.g., `"deleted"`) now returns 400 INVALID_BODY at the handler instead of producing per-lot ILLEGAL_TRANSITION rows.
> - **Extracted `api/_lib/pg-errors.ts`** with `pgCodeOf`, `PG_UNIQUE_VIOLATION`, `PG_FK_VIOLATION` — `lots/index.ts` and `lots/[id]/move.ts` were retrofitted to use the shared helper. Future write-endpoints reuse from here.
> - Tests expanded from 4 (no `move` coverage at all) to 14 across 4 describe blocks, with explicit savepoint-isolation coverage via the `INVALID_DESTINATION` test.
> - Actual commit message used: `feat(api): POST /api/lots/bulk (change-state, move, delete) with savepoint isolation` — slight rewording from the plan text above to surface the load-bearing implementation choice.
...
> **Implemented as commit `fede716` with one extension:** the export accepts the same filter set as `/api/lots` GET (`aiStatus[]`, `dateFrom`, `dateTo` added — plan only listed `customerId`, `jobId`, `state[]`). Without this parity, a user filtering inventory by date in the UI would export a mismatched dataset. Also added 4 more tests (jobId filter, state filter, warehouse 403, GET 405) for total of 6.
...
- [ ] **2. Add a "Lot detail" section to `src/routes/DesignSystem.tsx`** rendering one mock lot in each variant (assigned editable, sold frozen, picked-up frozen). Use static mock LotDTO objects since this page doesn't fetch real data. *(D-001 amended 2026-05-01: sold is now frozen — see Phase 2 spec §4.1 note.)*
...
<Section title="Lot detail — sold (frozen, amended D-001)">
...
- Assigned and unassigned variants show the form with editable fields
