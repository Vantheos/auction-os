...
- **`label_reprint_needed` is the source of truth for "this lot's printed label is stale."** It's set whenever any of the three printed-label fields (`lot.lot_number`, the lot's `customer.name`, the lot's `job.job_number`) changes; cleared whenever the lot's label is rendered again or the lot leaves a labelable state.
  - **Set sites** (in their respective transactions):
    - [`POST /api/jobs/:id/compact-lots`](../api/jobs/[id]/compact-lots.ts) — every lot whose `lot_number` shifts.
    - [`POST /api/lots/[id]/move`](../api/lots/[id]/move.ts) — single-lot move.
    - [`POST /api/lots/bulk` action=`move`](../api/lots/bulk.ts) — every successfully-moved lot in the per-lot SAVEPOINT (failed savepoints leave the flag unchanged).
    - [`PATCH /api/customers/[id]`](../api/customers/[id].ts) when `name` actually differs from the existing row — cascade `UPDATE` against every lot whose job belongs to this customer. Same-value PATCH is a no-op for the flag.
    - [`PATCH /api/jobs/[id]`](../api/jobs/[id].ts) when `jobNumber` actually differs — cascade `UPDATE` against every lot in this job. Same-value PATCH is a no-op.
  - **Clear sites:**
    - [`POST /api/labels/render`](../api/labels/render.ts) — clears the flag on a successful render (deliberate simplification, see below).
    - [`stateTransitionFields(to)`](../api/_lib/lot-state.ts) — when the target state is `unassigned` or `not-sellable`, the helper clears the flag alongside `jobId` / `lotNumber` per the `state_tuple_consistent` CHECK. A lot with no `lot_number` cannot be rendered (`/api/labels/render` returns 422 NOT_LABELLABLE) so the flag has no actionable meaning on it. If the lot later transitions back to `assigned` via Move, the move re-sets the flag — round-trip stays consistent.
  - **UI surfaces:** `<ReprintPill>` in inventory rows + a `?reprintPending=true` filter chip (desktop + mobile sheet) + a "Reprint labels" bulk action that runs `useBulkLabelPrint` over the selection.
  - **Convenience-checkbox semantics:** `MoveLotDialog` (single-lot move) and `BulkMoveDialog` (bulk move) both render a "Reprint labels" checkbox (default checked). The server flags moved lots **unconditionally** in Areas A above; the checkbox is a *convenience accelerator* that fires immediate prints right after the move so the operator doesn't have to chase the Reprint pending filter. With the checkbox unchecked, moved lots still surface via the pill / filter — no signal lost.
  - **Why `/api/labels/render` clears the flag without confirmation that the printer actually produced a label:** Browser Print's `POST /write` returns 200 when the ZPL is *accepted into its queue*, not when the printer produced the physical label. There is no callback for "label exited the printer." Any client-side "wait for confirmation" would be theater. So the server clears optimistically on render, and bulk operations end with a [`BulkPrintVerifyDialog`](../src/components/labels/BulkPrintVerifyDialog.tsx) popup explicitly asking the operator to verify the printed stack and reprint missing ones individually or via the filter. **Don't try to be cleverer than the hardware allows** — if you reach for "we'll detect failure and re-flag," the failure mode you're trying to detect is undetectable from our side.
...
**Reading per-lot duration** — claim → finalize timestamp gap is the wall-clock time including Supabase URL signing, the Anthropic call, and the finalize transaction. The Anthropic call is bounded by `PER_CALL_TIMEOUT_MS = 180_000` ([anthropic.ts](../src/lib/ai/anthropic.ts)) — we deliberately do NOT retry our own timeouts (slow calls don't get faster on retry, just double token cost). One outer retry remains for genuine transient infra errors (5xx/408) with a 1.5s backoff. The Anthropic SDK's default `maxRetries: 2` is left enabled — it handles 429 rate-limit responses by reading the `Retry-After` header and waiting the indicated interval before retrying, which is the only correct way to drain a TPM bucket. Do NOT pass `maxRetries: 0` to the SDK; it caused 8/11 fail-fast on 429s during 2026-05-08 testing.

**Anthropic rate limit interaction** — the org's input-TPM limit on claude-sonnet-4-6 is 30,000. Each call is roughly 5-15K input tokens (system prompt is ephemeral-cached after the first call; per-photo image tokens dominate after that). With `CONCURRENCY = 3`, three simultaneous lots can burst 15-45K tokens in <1s — touching or exceeding the cap. The SDK's `Retry-After` backoff handles this correctly when left at default. If 429s become a sustained problem, lower `CONCURRENCY` rather than re-disabling SDK retries.

**Worst-case budget per lot** — 180s first attempt + SDK internal retries (each can wait several seconds for `Retry-After`) + 1.5s outer backoff + 180s outer retry. Bounded loosely by the function maxDuration (300s). For a batch where every call hits 180s, with concurrency 3, expect to strand `N - 3` lots in claimed-but-not-finalized state for the per-lot stale window (5 min), then re-eligible. On prod, the cron tick continues the drain automatically (`ai_drain_in_progress = true` is set during the original drain). On preview, the operator clicks Run Now again. Plan accordingly when sizing tests.
