...
**Reading per-lot duration** — claim → finalize timestamp gap is the wall-clock time including Supabase URL signing, the Anthropic call, and the finalize transaction. The Anthropic call is bounded by `PER_CALL_TIMEOUT_MS = 180_000` ([anthropic.ts](../src/lib/ai/anthropic.ts)) — we deliberately do NOT retry our own timeouts (slow calls don't get faster on retry, just double token cost). One outer retry remains for genuine transient errors (5xx/429/408) with a 1.5s backoff, so the worst case for a 5xx-stricken call is ~180s + 1.5s + 180s ≈ 362s. SDK-internal retries are disabled (`maxRetries: 0`) so they don't compound. Concurrency is 3, function maxDuration is 300s — so an N-lot batch where every call hits the 180s ceiling would strand `N - 3` lots in claimed-but-not-finalized state for the per-lot stale window (5 min), then become re-eligible. On prod, the cron tick continues the drain automatically (`ai_drain_in_progress = true` is set during the original drain). On preview, the operator clicks Run Now again. Plan accordingly when sizing tests.
